...
`theme` definition is in `_scoring_rules.md` §0. Short summary: a 2–6 word descriptive phrase from the JD (e.g. "Bayesian Experimentation & Causal Inference", "AV V&V Safety Validation"). Used for recency display AND `excluded_areas` substring matching.

SCORING RULES (summary — canonical rules are in `.claude/_scoring_rules.md` which you already read above):
